...
function applyRuntimeSnapshot(snapshot) {
  if (!snapshot) return;
  window.TAR_SNAPSHOT = snapshot;
  RUNTIME_KEYS.forEach((key) => {
    if (Array.isArray(snapshot[key]) && Array.isArray(window[key])) {
      window[key].splice(0, window[key].length, ...snapshot[key]);
    }
  });
  if (snapshot.PAPER_SIGNAL && typeof snapshot.PAPER_SIGNAL === "object" && typeof window.PAPER_SIGNAL === "object") {
    Object.keys(window.PAPER_SIGNAL).forEach((key) => delete window.PAPER_SIGNAL[key]);
    Object.assign(window.PAPER_SIGNAL, snapshot.PAPER_SIGNAL);
  }
}

function useRuntimePolling(intervalMs = 5000) {
  const [state, setState] = React.useState({
    lastSync: window.TAR_SNAPSHOT?.generated_at || null,
    refreshing: false,
    error: null,
    version: 0,
  });
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      setState((current) => ({
        lastSync: snapshot.generated_at || new Date().toISOString(),
        refreshing: false,
        error: null,
      ...
      setState((current) => ({
        ...current,
        refreshing: false,
        error: err?.message || "snapshot unavailable",
      }));
    }
  }, []);
...
function syncLabel(lastSync, refreshing, error) {
  if (refreshing) return "syncing...";
  if (error) return `sync error · ${error}`;
  if (!lastSync) return "waiting for sync";
  const ts = Date.parse(lastSync);
  if (Number.isNaN(ts)) return "synced";
  const seconds = Math.max(0, Math.round((Date.now() - ts) / 1000));
  if (seconds < 2) return "last sync · now";
  if (seconds < 60) return `last sync · ${seconds}s ago`;
  return `last sync · ${Math.round(seconds / 60)}m ago`;
}

async function postRuntimeAction(path, payload = {}) {
...
function App() {
  ...
  const runtime = useRuntimePolling(5000);
  const [actionState, setActionState] = React.useState({ busy: false, message: null, error: null });
  ...
  async function runAction(path, payload, label) {
    setActionState({ busy: true, message: label, error: null });
    try {
      const result = await postRuntimeAction(path, payload);
      setActionState({ busy: false, message: `${label} queued`, error: null });
      await runtime.refresh();
      return result;
    } catch (err) {
      setActionState({ busy: false, message: null, error: err?.message || "action failed" });
      throw err;
    }
  }
      ...
      <main className="main">
        <div className="page-head">
          ...
          <div className="actions">
            <span className="timestamp" title={runtime.error || runtime.lastSync || ""}><span className="dot" />{syncLabel(runtime.lastSync, runtime.refreshing, runtime.error)}</span>
            <button className="icon" title="Refresh" onClick={runtime.refresh} disabled={runtime.refreshing}>↻</button>
          </div>
        </div>

        {page === "dashboard" && (
          <PageDashboard
            key={`dashboard-${runtime.version}`}
            tweaks={t}
            onNavigate={navigate}
            onAction={runAction}
        ...
        {page === "explorer" && <PageExplorer key={`explorer-${runtime.version}`} onNavigate={navigate} />}
        {page === "detail" && <PageDetail key={`detail-${runtime.version}`} strategy={selectedStrategy} onNavigate={navigate} />}
