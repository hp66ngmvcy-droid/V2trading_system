...
from tar_system.controller.job_queue import active_job_keys, add_job, make_active_job_key
...
def scan_raw_data(
    ...
    for path in sorted(Path(raw_dir).glob("*.csv")):
        ...
        stage_skip_walk_forward = research_stage == "smoke" if skip_walk_forward is None else skip_walk_forward
